  ...
  return (
    <div className="card p-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-line bg-ivory p-4"><span className="text-xs text-subdued">Stress agreement</span><strong className="mt-1 block text-lg text-ink">{pct(baseline.stress_agreement)}</strong></div>
        <div className="rounded-2xl border border-line bg-ivory p-4"><span className="text-xs text-subdued">Disagreement rate</span><strong className="mt-1 block text-lg text-ink">{pct(baseline.disagreement_rate)}</strong></div>
        <div className="rounded-2xl border border-line bg-ivory p-4"><span className="text-xs text-subdued">Latest baseline</span><strong className="mt-1 block text-lg text-ink">{baseline.latest_label}</strong></div>
      </div>
        ...
        <div className="mt-5 rounded-2xl border border-line bg-white p-4">
          ...
          <div className="space-y-2 text-xs text-muted">
            {baseline.disagreement_segments.map((seg, idx) => (
              <div key={idx} className="flex flex-wrap justify-between gap-2 rounded-xl bg-ivory px-3 py-2">
