  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[.8fr_1.2fr]">
      <div className="card p-6">
        ...
        <p className="mt-3 text-sm leading-6 text-slate-500">{projectCard?.tagline || 'From noisy market time series to structured regime intelligence.'}</p>
        <p className="mt-5 text-sm leading-7 text-slate-600">{projectCard?.description}</p>
        <div className="mt-5 flex flex-wrap gap-2">
          {(projectCard?.stack || []).map((item) => <span key={item} className="rounded-lg bg-blue-50 px-2.5 py-1 text-xs font-medium text-brand">{item}</span>)}
        </div>
        <div className="mt-5 rounded-2xl border border-line bg-slate-50 p-4 text-xs leading-5 text-slate-500">
