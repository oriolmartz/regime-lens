export default function ModelCardPanel({ modelCard }) {
  ...
  return (
    <div className="card p-6">
      ...
      <p className="mt-2 text-sm leading-6 text-muted">{modelCard.intended_use}</p>

      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <section className="rounded-2xl border border-line bg-ivory p-4">
          <h4 className="mb-2 text-sm font-semibold text-ink">Inputs</h4>
          <div className="flex flex-wrap gap-2">
            {(modelCard.inputs || []).map((item) => (
              <span key={item} className="rounded-lg bg-white px-2 py-1 text-xs font-medium text-muted ring-1 ring-line">{item}</span>
            ))}
          </div>
        </section>
        <section className="rounded-2xl border border-line bg-ivory p-4">
      ...
      <div className="mt-4 rounded-2xl border border-line bg-white p-4">
        ...
        <ul className="grid gap-1 text-xs leading-5 text-muted md:grid-cols-2">
